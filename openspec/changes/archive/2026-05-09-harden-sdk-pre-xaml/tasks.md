...
- [x] 12.1 **Server stderr leakage during `initialize`.** `RunServeAsync` now caps min-level to `LogLevel.Warning`; `StartupLogging.CreateLogger` mirrors the same cap so plugin-host / vocab-computation startup chatter doesn't leak. The `InitializeTests.Initialize_against_Sample_returns_capabilities_within_timeout` integration test now passes with empty stderr.
- [x] 12.2 **Live-index router timing — scopes invisible during cold-indexing.** Split `LiveIndexService.OpenScopeAsync` into a `PrepareScopeAsync` phase (runs in `StartAsync` — opens store, registers host with router as `status="indexing"`, persists registry row) and a `RunInitialIndexAsync` phase (runs in `ExecuteAsync` — opens solution, cold-indexes, settles status to `ok` / `degraded`). `StartAsync` is sequenced before the MCP transport's hosted service via DI registration order, so the router is fully populated before requests arrive. `MultiScopeTests.ListScopes_tool_reports_every_configured_scope` now passes.
- [x] 12.3 **`find_definition` does not lazy-wait for scope readiness.** `ScopeHost` now exposes a `Ready` task completed when the host's initial bring-up settles into `ok` or `degraded` (via `MarkReady()` called from `RunInitialIndexAsync`'s finally). `ScopedExecution.WaitUntilReadyAsync` is invoked at the start of every per-host fan-out leg in both the `string` and `CallToolResult` overloads of `RunAsync`; tools that hit a still-indexing scope wait, bounded by the caller's `CancellationToken`. `ColdIndexTests.FindDefinition_resolves_known_fixture_symbol_after_initialize` now passes within the 2-minute `IndexAndQueryTimeout`.
