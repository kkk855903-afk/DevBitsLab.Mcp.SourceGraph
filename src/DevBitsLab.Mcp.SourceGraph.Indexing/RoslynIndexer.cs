...
public sealed class RoslynIndexer : IAsyncDisposable, ILanguageIndexer
{
    ...
    internal static bool IsBuildGeneratedDocument(Document document)
    {
        ...
        {
            // A changed or unavailable Roslyn internal fails closed.
        }

        // MSBuildWorkspace does not mark WPF markup-compiler Documents as generated.
        // Accept only the compiler's narrow checksum/header shape under the owning
        // project's obj directory; a generated-looking path alone is insufficient.
        return IsWpfMarkupCompilerOutput(document);
    }

    private static bool IsWpfMarkupCompilerOutput(Document document)
    {
        var path = document.FilePath;
        var projectPath = document.Project.FilePath;
        if (document.Project.Solution.Workspace is not MSBuildWorkspace
        ...
        var objDirectory = Path.GetFullPath(Path.Combine(projectDirectory, "obj"));
        ...
        return text.StartsWith("#pragma checksum ", StringComparison.Ordinal)
        ...
        CancellationToken ct)
    {
        ...
        var generatedDiscovery = await DiscoverSourceGeneratedDocumentsAsync(
            projects,
            ct).ConfigureAwait(false);
        var generatedUniverseComplete =
            generatedDiscovery.IsComplete
            && _probedFailedProjectIds.Count == 0;
        // Generated declarations from different projects can share canonical identities.
        // Publishing only the successful subset of an incomplete discovery may therefore move
        // last-good symbols between owner files. Retain the whole prior generated projection
        // until every project's generated universe is available.
        var generated = generatedUniverseComplete
            ? generatedDiscovery.Documents
                .Where(document =>
                    !_pathPolicy!.IsGeneratedDocumentExcluded(document.FilePath))
                .ToList()
            : [];
        var generatedFailures = generatedDiscovery.Failures.ToList();
        var ownerCollision = generated
            .GroupBy(GetGeneratedStoragePath, _pathComparer)
            .FirstOrDefault(group => group.Skip(1).Any());
        if (ownerCollision is not null)
        {
            generatedUniverseComplete = false;
            generated = [];
            AddFileFailure(
                generatedFailures,
                ownerCollision.Key,
